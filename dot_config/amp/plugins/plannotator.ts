...
export default function plannotatorAmpPlugin(amp: PluginAPI) {
  ...
  amp.registerCommand(
    ...
    async (ctx) => {
      ...
      const result = await runPlannotator(amp, ctx, ["annotate", "--json", ...args]);
...
async function runPlannotator(
  ...
  const cwd = await resolveCwd(amp, ctx);
...
export async function resolveCwd(amp: PluginAPI, ctx: CommandContext): Promise<string> {
  const explicitCwd = normalizeDirectory(process.env.PLANNOTATOR_CWD);
  if (explicitCwd) return explicitCwd;

  if (ctx.system.workspaceRoot) {
    const workspaceCwd = normalizeDirectory(amp.helpers.filePathFromURI(ctx.system.workspaceRoot));
